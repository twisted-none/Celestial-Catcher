...
use crate::domain::IssApiResponse; // Используем новую структуру
use sqlx::PgPool;
use serde_json::json;
...
pub async fn start_background_collector(pool: Arc<PgPool>) {
    ...
    let source_url = "https://api.wheretheiss.at/v1/satellites/25544"; // Пример URL

    loop {
        interval.tick().await;
        
        // Генерируем фейковый JSON, похожий на реальный ответ
